...
export default function Neptune({ planetInfo }: Props) {
...
const ButtonContainer = styled.div<{ path: string }>`
  ...
  @media (min-width: 768px) {
    ...
    .over {
      background-color: ${(p) =>
        p.path === "/neptune" ? "#2D68F0" : "transparent"};
    }

    .struc {
      background-color: ${(p) =>
        p.path.includes("structure") ? "#2D68F0" : "transparent"};
    }

    .surf {
      background-color: ${(p) =>
        p.path.includes("surface") ? "#2D68F0" : "transparent"};
    }
  }
