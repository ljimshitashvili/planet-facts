...
export default function Uranus({ planetInfo }: Props) {
...
const ButtonContainer = styled.div<{ path: string }>`
  ...
  @media (min-width: 768px) {
    ...
    .over {
      background-color: ${(p) =>
        p.path === "/uranus" ? "#1EC1A2" : "transparent"};
    }

    .struc {
      background-color: ${(p) =>
        p.path.includes("structure") ? "#1EC1A2" : "transparent"};
    }

    .surf {
      background-color: ${(p) =>
        p.path.includes("surface") ? "#1EC1A2" : "transparent"};
    }
  }
