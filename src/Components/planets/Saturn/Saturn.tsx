...
export default function Saturn({ planetInfo }: Props) {
...
const ButtonContainer = styled.div<{ path: string }>`
  ...
  @media (min-width: 768px) {
    ...
    .over {
      background-color: ${(p) =>
        p.path === "/saturn" ? "#CD5120" : "transparent"};
    }

    .struc {
      background-color: ${(p) =>
        p.path.includes("structure") ? "#CD5120" : "transparent"};
    }

    .surf {
      background-color: ${(p) =>
        p.path.includes("surface") ? "#CD5120" : "transparent"};
    }
  }
